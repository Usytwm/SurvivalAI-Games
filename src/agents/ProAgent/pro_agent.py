...
class ProAgent(Agent_with_Memories):
    ...
    def decide_actions_for_next_turn(self):
        ...
        self.association_proposals_planned = (
            self.decide_association_proposals_for_next_turn(agents_to_extorsion)
        )

    def move(self, possible_moves: List[Tuple[int]]) -> Tuple[int]:
        destinations = [
            ((self.position[0] + move[0], self.position[1] + move[1]), move)
            for move in possible_moves
        ]
        destinations = [
            (self.evaluate_position(position), move) for position, move in destinations
        ]
    ...
    def actualize_agresivities(self) -> None:
        for agent_id in self.memory_for_agents_sights.agents_seen:
            self.computated_agresivities[agent_id] = (
                self.memory_for_attacks.attacks_per_agent.get(agent_id, 0)
                / self.memory_for_agents_sights.agents_seen[agent_id]
            )

    def evaluate_position(self, position: Tuple[int, int]) -> float:
        sugar = self.geographic_memory.get_last_info_of_sugar_in_position(
    ...
    def get_risk_of_position(self, position: Tuple[int, int]) -> float:
        ...
        for agent_id in self.memory_for_agents_sights.agents_seen:
            if agent_id in self.memory_for_attacks.deaths:
                continue
            other_agent_position, _, other_agent_sugar = (
                self.memory_for_agents_sights.get_last_info_from_agent(agent_id)
            )
            distance = max(
                abs(position[0] - other_agent_position[0])
                + abs(position[1] - other_agent_position[1]),
                1,
            )
    ...
    def decide_association_proposals_for_next_turn(
        self, agents_to_extorsion: List[int]
    ) -> List[Association_Proposal]:
        ...
        for victim_id in agents_to_extorsion:
            ...
            association_proposals.append(
                Association_Proposal(victim_id, [self.id, victim_id], commitments)
            )
        direct_threats: List[Tuple[int, int, int]] = (
            []
        )  # [(other_id, other_resources, other_distance)]
        par_agents: List[Tuple[int, int, int]] = (
            []
        )  # [(other_id, other_resources, other_distance)]
        for other_id in self.memory_for_agents_sights.agents_seen:
            if (
                other_id in self.memory_for_attacks.deaths
                or other_id in agents_to_extorsion
            ):
                continue
            other_position, _, other_resources = (
                self.memory_for_agents_sights.get_last_info_from_agent(other_id)
            )
            distance = abs(other_position[0] - self.position[0]) + abs(
                other_position[1] - self.position[0]
            )
            if (
                (distance <= 3)
                and (other_resources > self.reserves)
                and self.computated_agresivities[other_id] >= 0.25
            ):
        ...
        direct_threats.sort(
            key=lambda tpl: (self.computated_agresivities[tpl[0]], tpl[2]), reverse=True
        )
        ...
        for other_id, other_resources, distance in direct_threats:
            ...
            association_proposals.append(
                Association_Proposal(self.id, [self.id, other_id], commitments)
            )
            idx += 1
        par_agents = [
            (id, self.kill_apeal_formula(distance, self.computated_agresivities[id]))
            for id, _, distance in par_agents
        ]
    ...
    def decide_attacks_for_next_turn(self) -> Tuple[List[Attack], List[int]]:
        ...
        attacks = []
        killable_agents: List[Tuple[int, int, float]] = (
            []
        )  # id, fuerza_requerida, formulita
        for other_id in self.memory_for_agents_sights.agents_seen:
            if other_id in self.memory_for_attacks.deaths:
                continue
            is_vulnerable, strength_needed, target_apeal = self.evaluate_attack_to_kill(
                other_id
            )
            if is_vulnerable:
    ...
    def evaluate_attack_to_kill(self, other_id) -> Tuple[bool, int, float]:
        ...
        other_expected_resources = (
            other_resources
            + self.geographic_memory.get_max_sugar_around_position(
        ...
        if other_expected_resources < self.reserves:
            return (
                True,
                other_expected_resources,
                self.kill_apeal(other_id, other_position),
            )
    ...
    def get_fractions_to_sacrifice(self, direct_threats: List[Tuple[int, int, int]]):
        ...
        if (len(direct_threats) < 3) and (
            self.free_portion - 0.3 > self.minimun_free_portion
        ):
            return [0.2, 0.1]
        for i in range(len(direct_threats)):
            fractions_to_sacrifice.append(
                min(0.1, self.free_portion - self.minimun_free_portion)
            )
    ...
    def consider_association_proposal(self, proposal: Association_Proposal) -> bool:
        ...
        for other_id in proposal.members:
            ...
            distance = abs(other_position[0] - self.position[0]) + abs(
                other_position[1] - self.position[1]
            )
            if (
                (other_resources > self.reserves)
                and (distance <= 3)
                and (
                    self.free_portion - proposal.commitments[self.id][0]
                    > self.minimun_free_portion
                )
            ):
                return True
            is_killable, strength_needed, target_apeal = self.evaluate_attack_to_kill(
                other_id
            )
            if (
                is_killable
                and (self.reserves - strength_needed) / risk > self.security_umbral
            ):
