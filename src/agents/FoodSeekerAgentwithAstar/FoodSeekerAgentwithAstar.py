...
class FoodSeekerAgentwithAstar(Agent_with_Memories):
    ...
    def __init__(self, id, consume: int, reserves, conn: sqlite3.Connection):
        ...
        self.path_finder = PathFinder(self.geographic_memory.validate_position, self.heuristic, self.cost_function)
    ...
    def cost_function(self, A : Tuple[int, int], B : Tuple[int, int], goal : Tuple[int, int]) -> float:
        numerador = 1
        ...
        try:
            ...
            denominador = max(self.geographic_memory.get_last_info_of_sugar_in_position(B[0], B[1])[1], 1)
        except:
            denominador = 1
