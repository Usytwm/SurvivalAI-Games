...
def analyze_associations_correlation(df):
    ...
    plt.title("Correlación entre Número de Asociaciones y Victorias por Tipo de Agente")
...
def analyze_reserves_change(df):
    ...
    plt.figure(figsize=(10, 6))
    sns.stripplot(
        data=df_sorted, x="AgentType", y="ReserveChange", jitter=True, size=5, alpha=0.6
    )
...
def calculate_association_metrics(results_df):
    ...
    association_stats = results_df["HasAssociations"].value_counts(normalize=True) * 100
    ...
    ax.set_xticklabels(["Con Asociaciones", "Sin Asociaciones"], rotation=0)
    plt.show()
...
results_df = run_multiple_simulations(1000, 20, 20, 100)
