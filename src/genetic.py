...
def crear_f_poblacion_inicial(N, K):
    ...
    for _ in range(N):
        f = generar_f(
            K, 6
        )  # Este parámetro 6 que se están tomando 7 valores de la simulación para cambiar de estrategia
    ...
    for i in range(len(adn_poblacion), extras):
        ...
        handler = random.choice(
            [
                ...
                PacifistAgent(agent_id, consume, reserves, sqlite3.connect(":memory:")),
                FoodSeekerAgent(
                    agent_id, consume, reserves, sqlite3.connect(":memory:")
                ),
                RandomAgent(agent_id, consume, reserves, sqlite3.connect(":memory:")),
                CombatantAgent(
                    agent_id, consume, reserves, sqlite3.connect(":memory:")
                ),
            ]
...
def algoritmo_genético(tamaño_población, generaciones):
    ...
    adn_poblacion = reproducir(adn_poblacion, tamaño_población)  # Devuelve solo el ADN
    adn_optimo = []
